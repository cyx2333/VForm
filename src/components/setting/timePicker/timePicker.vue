<template>
  ...
  > 
    ...
    <a-form-item label="显示格式" name="valueFormat" >
      <a-select v-model:value="element.options.valueFormat" size="small" >
        <a-select-option value="HH:mm:ss">HH:mm:ss</a-select-option>
        <a-select-option value="HH时mm分ss秒">HH时mm分ss秒</a-select-option>
        <a-select-option value="hh:mm:ss">hh:mm:ss</a-select-option>
      </a-select>
    </a-form-item>
    <a-divider>时间属性</a-divider>
    <a-form-item :wrapperCol="{span: 24}" :labelCol="{span: 24}" label="默认时间" name="range" >
      <a-time-picker v-model:value="element.options.defaultValue" allowClear :valueFormat="element.options.valueFormat" v-if="!element.options.range" style="width: 100%;" />
      <a-time-range-picker allowClear :valueFormat="element.options.valueFormat" v-if="element.options.range" @change="rangePickerChange"/>
    </a-form-item>
  </a-form>
...
const rangeChange = (status) => {
  if (status) {
    element.options.wrapperSpan = 8
    element.options.defaultValue = WidgetData.createReactive([])
  } else {
    element.options.wrapperSpan = 5
